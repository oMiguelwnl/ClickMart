...
const Checkout = () => {
  ...
  const paymentSubmit = () => {
    if (
      ...
      localStorage.setItem("ÚltimoPedido", JSON.stringify(orderData));
      navigate("/payment");
    }
  };
    ...
    await axios.get(`${server}/coupon/get-coupon-value/${name}`).then((res) => {
      ...
      if (res.data.couponCode !== null) {
        ...
        if (isCouponValid.length === 0) {
          toast.error("Código do cupom não é válido para esta loja");
          setCouponCode("");
        } else {
...
const ShippingInfo = ({
  ...
  return (
    <div className="w-full 800px:w-[95%] bg-white rounded-md p-5 pb-8">
      <h5 className="text-[18px] font-[500]">Informações de Entrega</h5>
      <br />
      <form>
      ...
      >
        Escolher a partir do endereço salvo
      </h5>
      {userInfo && (
        <div>
          {user &&
